...
interface Stats {
  ...
  ipv6_blocked: number;
  fingerprints_randomized: number;
  last_fingerprint_at: number | null;
  last_fingerprint_user_agent: string | null;
...
function App() {
  const [stats, setStats] = useState<Stats>({
    ...
    last_fingerprint_user_agent: null,
    total_requests: 0,
    proxy_running: false,
  ...
  const [targetConnectionState, setTargetConnectionState] = useState<boolean | null>(null);
    ...
    const fetchStats = async () => {
      try {
        ...
        setStats((prev) => ({
          ...prev,
          ...data,
          fingerprints_randomized: data.fingerprints_randomized ?? 0,
          last_fingerprint_at: data.last_fingerprint_at ?? null,
          last_fingerprint_user_agent: data.last_fingerprint_user_agent ?? null,
        }));
      } catch (error) {
  ...
  // Listen for native close dialog
  useEffect(() => {
    const setupCloseListener = async () => {
      const unlisten = await listen('show-close-dialog', () => {
  ...
  const handleCloseMinimize = async () => {
    setShowCloseDialog(false);
    await invoke('close_window_only');
  };

  const handleCloseShutdown = async () => {
    setShowCloseDialog(false);
    await invoke('shutdown_and_exit');
  };

  const toggleConnection = async () => {
    const targetState = !stats.proxy_running;
    setTargetConnectionState(targetState);
    ...
    try {
      ...
      const data = await response.json();
      const connected = data.is_connected || data.tor_connected;
      
      setStats((prev) => ({ ...prev, tor_connected: connected }));
      
      if (!targetState) {
        setIsConnecting(false);
        setTargetConnectionState(null);
      }
    } catch (error) {
  ...
  const protectionReady = stats.proxy_running && stats.tor_connected;

  useEffect(() => {
    if (isConnecting && targetConnectionState === true && protectionReady) {
      setIsConnecting(false);
      setTargetConnectionState(null);
    }
  ...
  const toggleKillSwitch = async () => {
    try {
      const response = await fetch("http://127.0.0.1:3030/api/killswitch", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      const data = await response.json();
      setStats((prev) => ({ ...prev, kill_switch_active: data.kill_switch_active }));
    } catch (error) {
  ...
  const formatSince = (unixSeconds: number | null) => {
    if (!unixSeconds) return 'No fingerprint yet';
    const now = Date.now();
  ...
  const truncateUa = (ua: string | null) => {
    if (!ua) return 'UA: —';
    return ua.length > 48 ? `UA: ${ua.slice(0, 48)}…` : `UA: ${ua}`;
  };

  const runSelfTest = async () => {
    if (selfTestRunning) return;
    setSelfTestRunning(true);
    setSelfTestStatus(null);
    setSelfTestChecks([]);
    setShowSelfTestPanel(true);
    try {
      const response = await fetch("http://127.0.0.1:3030/api/self-test", {
        method: "POST",
      });
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}`);
      ...
      setSelfTestChecks(checks);
      const failures = checks.filter((c: any) => !c.ok);
      if (data.ok) {
        setSelfTestStatus({ ok: true, message: "Self test passed" });
      } else if (failures.length > 0) {
        const first = failures[0];
        const detail = first?.detail ? `: ${first.detail}` : '';
        setSelfTestStatus({ ok: false, message: `Self test failed: ${first.name}${detail}` });
      } else {
        setSelfTestStatus({ ok: false, message: "Self test failed" });
      }
    } catch (error) {
      console.error("Self test failed:", error);
      setSelfTestStatus({ ok: false, message: "Self test unavailable" });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f1624] text-white flex flex-col relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 py-6 max-w-lg flex flex-col flex-1 relative z-10">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-xl transition-all flex items-center space-x-2 ${
                selfTestRunning
                  ? 'bg-white/5 text-gray-400'
                  : 'bg-gradient-to-r from-emerald-500/15 to-teal-500/15 hover:from-emerald-500/25 hover:to-teal-500/25 text-emerald-300'
              }`}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6-1a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span className="text-sm font-medium">Self Test</span>
            </button>
            <button
              onClick={() => setShowDonateModal(true)}
              className="px-3 py-2 bg-gradient-to-r from-purple-500/15 to-pink-500/15 hover:from-purple-500/25 hover:to-pink-500/25 rounded-xl transition-all flex items-center space-x-2"
            >
        ...
        {showSelfTestPanel && (selfTestRunning || selfTestStatus || selfTestChecks.length > 0) && (
          <div className="mb-4 flex items-center justify-center">
            <div className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3">
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold text-purple-200">Self Test</div>
                <button
                  onClick={() => setShowSelfTestPanel(false)}
              ...
              {selfTestChecks.length > 0 && (
                <div className="mt-3 space-y-2">
                  {selfTestChecks.map((check, index) => (
                    <div key={`${check.name}-${index}`} className="flex items-center justify-between text-xs">
                      <div className="text-gray-300">
                        {check.name}
                        {check.detail ? <span className="text-gray-500"> — {check.detail}</span> : null}
                      </div>
                      <div className={`font-semibold ${check.ok ? 'text-emerald-300' : 'text-red-300'}`}>
                        {check.ok ? 'OK' : 'FAIL'}
                      </div>
                    </div>
                  ))}
          ...
          <div className="flex flex-col flex-1">
            <div className="space-y-4 mb-6">
              <div className="flex justify-center">
                <div className={`inline-flex items-center space-x-2 px-5 py-2.5 rounded-full backdrop-blur-xl ${
                  ...
                  <div className={`w-2 h-2 rounded-full ${protectionReady ? 'bg-emerald-400 shadow-lg shadow-emerald-400/50' : 'bg-gray-500'}`}></div>
                  <span className="text-sm font-semibold tracking-wide">
                    {protectionReady ? 'PROTECTED' : 'DISCONNECTED'}
                  </span>
                </div>
              </div>
            ...
            <div className="flex justify-center items-center flex-1">
              <button
                ...
                disabled={isConnecting || (stats.proxy_running && !protectionReady)}
                className={`relative group w-48 h-48 rounded-full transition-all duration-700 transform ${
                  isConnecting || (stats.proxy_running && !protectionReady) ? 'scale-95 opacity-75' : 'hover:scale-105'
                } ${
                  protectionReady
              ...
              >
                <div className="absolute inset-2 rounded-full bg-[#1a1a2e]/90 backdrop-blur-md flex items-center justify-center border-2 border-white/10">
                  {isConnecting || (stats.proxy_running && !protectionReady) ? (
            ...
            <div className="space-y-3 px-6 pb-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-sm font-semibold text-purple-300 uppercase tracking-wide">Statistics</h3>
                  <button
                    onClick={() => setStatsCollapsed(!statsCollapsed)}
                    className="p-1.5 bg-white/5 hover:bg-white/10 rounded-lg transition-all"
                  >
                    <svg 
                      className={`w-4 h-4 text-gray-300 transition-transform ${statsCollapsed ? 'rotate-180' : ''}`} 
                      fill="none" 
                      stroke="currentColor" 
                      viewBox="0 0 24 24"
                    >
                ...
                {!statsCollapsed && (
                  <>
                    ...
                    <button 
                      ...
                      className="w-full px-4 py-3 bg-gradient-to-r from-fuchsia-500/15 to-purple-500/15 rounded-xl backdrop-blur-xl border border-fuchsia-500/30 hover:border-fuchsia-400/50 hover:shadow-lg hover:shadow-fuchsia-500/20 transition-all cursor-pointer flex items-center justify-between group"
                    >
                      <div className="flex items-start space-x-3">
                        <svg className="w-5 h-5 text-fuchsia-400 group-hover:text-fuchsia-300 transition-colors mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 11c0 3.866-3.582 7-8 7a9 9 0 0116 0c-4.418 0-8-3.134-8-7z" />
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 11a4 4 0 100-8 4 4 0 000 8z" />
                        </svg>
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-fuchsia-300 uppercase tracking-wide">Fingerprint</span>
                          <span className="text-xs text-fuchsia-300/70">{formatSince(stats.last_fingerprint_at)}</span>
                          <span className="text-[10px] text-fuchsia-300/50 mt-0.5">{truncateUa(stats.last_fingerprint_user_agent)}</span>
                        </div>
                      </div>
                      <span className="text-xl font-bold text-fuchsia-300">{stats.fingerprints_randomized.toLocaleString()}</span>
                    </button>
        ...
        {showCloseDialog && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <div className="w-full max-w-sm rounded-2xl bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f1624] text-white shadow-2xl overflow-hidden border border-purple-500/20">
              <div className="px-6 pt-6 pb-4 flex flex-col items-center text-center">
                <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-yellow-400/10 border border-yellow-400/30">
                  <svg className="mx-auto h-7 w-7 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                </div>
                <h2 className="text-lg font-semibold">Close Privacy Suite?</h2>
              ...
              <div className="border-t border-white/10">
                <div className="grid grid-cols-2 divide-x divide-white/10">
                  <button
                    ...
                    className="py-3 text-sm font-semibold text-blue-400 hover:bg-white/5 transition-colors"
                  >
                    Background
                  </button>
                  <button
                    onClick={handleCloseShutdown}
                    className="py-3 text-sm font-semibold text-red-400 hover:bg-white/5 transition-colors"
                  >
                    Exit
                  </button>
                </div>
